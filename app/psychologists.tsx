...
export default function PsychologistsScreen() {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/therapists/psychologists`);
    ...
    console.log('Psychologists from backend:', data);

    setTherapists(data);

    if (!Array.isArray(data) || data.length === 0) {
      setMessage('No Psychologists found.');
    }
  } catch (error: any) {
    console.error('Frontend fetch error:', error);
    setMessage(`Could not load Psychologists: ${error.message}`);
  } finally {
    setLoading(false);
  }
};
  ...
  const fetchByLocation = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/therapists/psychologists?lat=${latitude}&lng=${longitude}`
      );
      ...
      setMessage('Could not load Psychologists.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchByCity = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/therapists/psychologists?city=${encodeURIComponent(city)}`
      );
      ...
      if (data.length === 0) {
        setMessage('No Psychologists found for this city.');
      }
    } catch (error) {
      setMessage('Could not load Psychologists.');
    } finally {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.title}>Psychologists</Text>
        <Text style={styles.subtitle}>
          Browse Psychologists or sort them by your location
        </Text>
        ...
        {therapists.map((therapist) => (
          <View key={therapist.id} style={styles.card}>
            ...
            <Text style={styles.cardText}>Licensed Psychologist</Text>
